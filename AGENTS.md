...
- `bootstrap/linux.sh [target]` is for ordinary Linux/WSL, defaults to `xuqihao`, and runs seven steps: install Nix, configure cache trust, optionally store a GitHub token, enable flakes, back up conflicting files, install Home Manager, activate with `-b backup`. Do not use it for NixOS-WSL or for macOS. It also runs outside a checkout (the README curl one-liner or a standalone downloaded copy): it fetches the repo to `CLONE_DIR` (default `~/nix-roam`, overridable; git clone preferred, Gitee archive tarball when git is missing) and re-execs the in-repo copy. `linux.sh` additionally guards `uname -s` = Linux. Both standalone bootstraps abort up front when the target's username differs from the current login user (standalone Home Manager can only activate for the current user), with a hint pointing at the one-line `username` edit in `flake.nix` — this prevents the half-configured state of system nix settings being rewritten before activation fails on someone else's HOME.
